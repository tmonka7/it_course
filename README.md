...
Translations live in `frontend/src/i18n/zh/*.js` and `frontend/src/i18n/ja/*.js`, one file per area, keyed by the English text; a missing entry falls back to English. To add a language, copy one of those folders, translate the values, and register it in `frontend/src/i18n/index.jsx` (plus the antd locale in `main.jsx` and the `language` enum on the User model). Data entered by users, and notifications or activity entries generated by the server, stay in the language they were written in.
...
frontend/src
  ...
  i18n/                    t() / <T> helpers and the Chinese / Japanese dictionaries (zh/*.js, ja/*.js, keyed by English text)
